...
def _extract_words(text: str, language: str) -> list[str]:
    ...
    if language.lower() == "hebrew":
        ...
        # Latin scripts and Cyrillic (English, Spanish, French, etc.)
...
def chat(
    ...
    for attempt in range(max_retries):
        ...
        if not found_violations:
            ...
            # First attempt had violations — give the AI one retry hint
            violations = found_violations
            final_response = candidate
            attempts_used = attempt + 1
        else:
            # After retry the AI still wants these words — accept as new vocabulary.
